<script lang="ts">
  import { ChevronDown, Pause, Play, RefreshCw, Search, ShieldCheck, Smartphone, Square, Wifi } from "lucide-svelte";
  import {
    chooseDevice,
    choosePackage,
    connectCompanion,
...
<header class="topbar">
  ...
  <div class="picker">
    ...
    {#if devicesOpen}<div class="menu device-menu">{#each devices as item (item.serial)}<button class:selected={device === item.serial} onclick={() => chooseDevice(item.serial)}><span><b>{item.model || item.serial}</b><small>{item.connection_type} · {item.authorization_status}</small></span></button>{/each}{#if !devices.length}<span class="menu-empty">No Android devices found</span>{/if}</div>{/if}
  </div>
