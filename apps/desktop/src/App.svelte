...
<main class="svelte-app">
  ...
  <section class:logs-mode={screen === "logs"} class="svelte-main">
    ...
    {:else}
      ...
      <section class="log-feed">{#each incidents as issue (issue.signature)}{@const expanded = expandedIssue === issue.signature}<article class:expanded class="issue-card"><div class="issue-kind"><AlertCircle size={18}/>{issue.category.replaceAll("_", " ")}</div><div class="issue-body"><button class="issue-toggle" aria-expanded={expanded} onclick={() => expandedIssue = expanded ? "" : issue.signature}><div class="issue-title"><div><h2>{issue.title}</h2><p>{issue.summary}</p></div><div class="issue-meta">{#if issue.occurrence_count > 1}<b class="occurrences">×{issue.occurrence_count}</b>{/if}<time>{new Date(issue.occurred_at).toLocaleTimeString()}</time><ChevronDown class="issue-chevron"/></div></div><div class="issue-context">{issue.where_occurred || issue.foreground_activity || "App context unavailable"}</div></button>{#if expanded}<div class="issue-details"><div class="issue-details-inner"><div class="issue-summary"><div><small>Summary</small><p>{issue.summary}</p></div><div><small>Root cause</small><p>{issue.root_cause || issue.likely_cause || "Still being classified from the captured evidence."}</p></div><div><small>How it happened</small><p>{issue.how_occurred || issue.likely_cause || "No additional execution context was recorded."}</p></div></div><div class="issue-evidence"><div><small>Reproduction / context</small><ol>{#each issue.reproduction_steps as step}<li>{step}</li>{/each}</ol></div><div><small>Captured logs</small><pre>{issue.lines.map(line => `${line.timestamp_ms}  ${line.level.padEnd(5)}  ${line.tag}: ${line.message}`).join("\n") || "No raw Logcat lines were retained for this issue."}</pre></div></div><button class="quiet" onclick={() => copy(issue.lines.map(line => `${line.level} ${line.tag}: ${line.message}`).join("\n"))}><Copy/>Copy logs</button></div></div>{/if}</div></article>{/each}{#if !incidents.length}<div class="empty-state log-empty"><ShieldCheck size={32}/><b>No issues detected</b><span>{capturing ? "App Tester is watching Logcat for actionable problems." : "Start monitoring to see errors and warnings here."}</span></div>{/if}</section>
    {/if}
  </section>
</main>
