...
```text
Human navigation
  -> USB ADB reverse tunnel and per-app Android VPN
...
`androidqa-core` owns all security-sensitive and analytical behavior. Svelte owns only view state, filters, selection, and explicit commands.
